...
def AnalyseThresholdScanV4(FileThresholdScan):
...
# -------------- ANALYSE TDAC THRESHOLD SCANS VERSION 2 FOR HVCMOS VERSION 4 ------------
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
# -------------- Functions to analyse several threshold scans with a different TDAC
#				and predict a TDAC for each pixel by INTERPOLATION
...
def AnalyseTDACThresholdScansV2V4(FileTDAC_list,TDAC_value_list, TargetThreshold):	# TODO : add area in inputs
...
#	injToElectrons=1660./.39			# WARNING: different depending the version of the chip: v2: 1660/0.39 v4: 1660/0.25
	...
	lenTDAClist = len(TDAC_value_list)+2
	
	for n, TDACcnt in zip(range(2,lenTDAClist), TDAC_value_list):
		FileDataThrScan = FileTDAC_list[n-2]
	...
	for r in range(12):
		for c in range(24,48):
			...
			if len(Diff_Abs_list) < 2:
				...
				second_point_TDAC = TDAC_list[Min_index-1]
				second_point_thresh = Thresh_list[Min_index-1]
			elif Diff_list[Min_index] > 0 and Min_index < 2:				# second selection: last point
				
				print r,c, Min_index, Diff_Abs_list
				
				if Min_index == len(Diff_list) -1:
					second_point_TDAC = -1					# < 3: 3 = nbr de TDAC thresh scan -1
					second_point_thresh = -1
				else:
					second_point_TDAC = TDAC_list[Min_index+1]					# < 3: 3 = nbr de TDAC thresh scan -1
					second_point_thresh = Thresh_list[Min_index+1]
			else:															# else put a value of -1 in order
				second_point_TDAC = -1										# to identify it easely later
				second_point_thresh = -1
			...
			Xaxis = g_Thr_vs_TDAC.GetXaxis()
			Xaxis.SetLimits(0.,16.)
			g_Thr_vs_TDAC.GetHistogram().SetMaximum(1000.)
			g_Thr_vs_TDAC.GetHistogram().SetMinimum(0.)
...
# Small area study: analyse VNDAC and Global threshold
#______________________________________________________________________________________________________
#______________________________________________________________________________________________________
#______________________________________________________________________________________________________

def AnalyseTDACThresholdScansCCPDv4SmallArea(FileTDAC_list,TDAC_value_list, TargetThreshold):	# TODO : add area in inputs
...
#	injToElectrons=1660./.39			# WARNING: different depending the version of the chip: v2: 1660/0.39 v4: 1660/0.25
	...
	for n, TDACcnt in zip(range(2,lenTDAClist), TDAC_value_list):
		...
		for r in range(1,11):	# modified to cope with smalla area: --> row 1 to 10
			for c in range(31,41):	# modified to cope with small area --> col 31 to 40
				TDAC_value = TDAC_value_dic["r"+str(r)+"_c"+str(c)+""]
				Threshold_value = Threshold_value_dic["r"+str(r)+"_c"+str(c)+""]
				Sigma_value = Sigma_value_dic["r"+str(r)+"_c"+str(c)+""]
				Chi2_value = Chi2_value_dic["r"+str(r)+"_c"+str(c)+""]
	...
	# ------- Find suggested TDAC and expected threshold with sugg TDAC			
	for r in range(1,11):	#instead of range(12) modified to cope with smalla area: --> row 1 to 10
		for c in range(31,41):			# instead of range(24,48) # modified to cope with smalla area: --> col 31 to 40
			x = []
			y = []
			Diff_Abs_list = []
			Diff_list = []
			...
			if y.count(0) >= len(y)-1:
				slope = 0
				p0 = 0
				
				for TDACcntDiff in TDAC_value_list:				
					AllDiffFit_dic["r"+str(r)+"_c"+str(c)+"_TDAC"+str(TDACcntDiff)+""] = 1000				
				
			else:
				if y.count(0) == 0:
					IndexRangeMax = len(y)-1
				else:
					IndexRangeMax = next(xl[0] for xl in enumerate(y) if xl[1] < 150 ) - 1 # -1 necessary
				
				print ""
				print "index range max = ", IndexRangeMax
				IndexRangeMin = next(xl[0] for xl in enumerate(y) if xl[1] > 150 )
				RangeMax = x[int(IndexRangeMax)]
				RangeMin = x[IndexRangeMin]
				print ""
				print " range max = ", RangeMax
				print " range min = ", RangeMin

				myfit_Thr_vs_TDAC=TF1("myfit_Thr_vs_TDAC", "pol1",RangeMin,RangeMax)
				g_Thr_vs_TDAC.Fit("myfit_Thr_vs_TDAC","R")
				p0 = myfit_Thr_vs_TDAC.GetParameter(0)
				SlopeOriginal = myfit_Thr_vs_TDAC.GetParameter(1)
				slope = SlopeOriginal*(-1)
				p0 = myfit_Thr_vs_TDAC.GetParameter(0)
				
				print ""
				print "SlopeOriginal = ", SlopeOriginal
				print "p0 = ", p0
				
				
				for TDACcntDiff in TDAC_value_list:				
					Thresh_value_diff = AllThresh_simpl_dic["r"+str(r)+"_c"+str(c)+"_TDAC"+str(TDACcntDiff)+""]
					DiffFit = Thresh_value_diff - (SlopeOriginal * float(TDACcntDiff)+ p0)
					AllDiffFit_dic["r"+str(r)+"_c"+str(c)+"_TDAC"+str(TDACcntDiff)+""] = DiffFit
					print ""
					print "Thresh value = ",Thresh_value_diff
					print "TDAC = ", TDACcntDiff
					print "DiffFit = ",DiffFit 
					
#			print AllDiffFit_dic
			print ""
#			print 
			...
			# outputs
			AllSlope_dic ["r"+str(r)+"_c"+str(c)+""] = slope
			Allp0_dic ["r"+str(r)+"_c"+str(c)+""] = p0			
			AllExpectThresh_dic ["r"+str(r)+"_c"+str(c)+""] = expected_Threshold
			AllSuggTDAC_dic ["r"+str(r)+"_c"+str(c)+""] = suggTDAC_arrondi			
			g_Thr_vs_TDAC.SetNameTitle("r"+str(r)+"_c"+str(c)+"_suggTDAC "+str(suggTDAC_arrondi)+"","r"+str(r)+"_c"+str(c)+" ")
			g_Thr_vs_TDAC.SetTitle("r"+str(r)+"_c"+str(c)+",  suggTDAC "+str(suggTDAC_arrondi)+", exp th = "+str(int(expected_Threshold))+", first TDAC = "+str(first_point_TDAC)+" , second TDAC = "+str(second_point_TDAC)+" , p = "+str(p)+" slope = "+str(slope)+"")		
			AllThr_vs_TDAC_graphs_dic ["r"+str(r)+"_c"+str(c)+""] = g_Thr_vs_TDAC
			g_Thr_vs_TDAC_List.append(g_Thr_vs_TDAC)

	return [AllThresh_dic, AllSigma_dic, AllChi2_dic, AllData_pointsX_dic, AllData_pointsY_dic, AllSuggTDAC_dic, AllExpectThresh_dic, AllThr_vs_TDAC_graphs_dic, AllThresh_simpl_dic, AllSlope_dic, Allp0_dic, AllDiffFit_dic]
